...
void initHashtable() {
	...
	FieldList f1 = malloc(sizeof(FieldList_));
	f1->type = malloc(sizeof(Type_));
	strcpy(f1->name, "read");
	f1->tail = NULL;
	f1->type->kind = FUNCTION;
	f1->type->u.function.retType = malloc(sizeof(Type_));
	...
	FieldList f2 = malloc(sizeof(FieldList_));
	f2->type = malloc(sizeof(Type_));
	strcpy(f2->name,"write");
	f2->tail = NULL;
	f2->type->kind = FUNCTION;
	f2->type->u.function.retType = malloc(sizeof(Type_));
	...
	strcpy(write_param->name, "write_param_name");
	write_param->tail = NULL;
	write_param->type = malloc(sizeof(Type_));
...
void ExtDecList(Node *root, Type type) {
	...
	if(t&&t->kind == ARRAY) {
		Operand Dec = calloc(1, sizeof(Operand_));
		Dec->kind = TEMP_VAR_OP;
		Dec->u.var_no = TempVarCount++;
		InterCode decIR = calloc(1, sizeof(InterCode_));
		decIR->kind = DEC_IR;
		decIR->u.decOP.op = Dec;
		decIR->u.decOP.size = getSize(t);
		insertCode(decIR);
		
		/*Operand ArrayDddr = calloc(1, sizeof(Operand_));
		ArrayDddr->kind = VARIABLE_OP;
		strcpy(ArrayDddr->u.value, t->name);
		InterCode addrIR = calloc(1, sizeof(InterCode_));
...
void Stmt(Node *root, Type type) {
	...
	if(!strcmp(child->name, "RETURN")) { //RETURN Exp SEMI
		...
		Operand Return = calloc(1, sizeof(Operand_));
		if(!strcmp(child->FirstChild->name, "INT")) {
			Return->kind = CONSTANT_OP;
			itoa(child->FirstChild->u.value_i, Return->u.value, 10);
		}
		else 
			Type temp = Exp(child, Return);
		InterCode returnIR = calloc(1, sizeof(InterCode_));
		returnIR->kind = RETURN_IR;
		returnIR->kind = u.unaryOP.op = Return;
		insertCode(returnIR);
		/*Type t = Exp(child);
		...
		Stmt(child, type);
		InterCode gotoIR = calloc(1, sizeof(InterCode_));
		gotoIR->kind = GOTO_IR;
		gotoIR->u.unaryOP.op = label1;
		insertCode(gotoIR);
		InterCode IR3 = calloc(1, sizeof(InterCode_));
		IR3->kind = LABEL_IR;
...
FieldList Dec(Node *root, Type type, int isFromStruct) {
	...
	if(child) { 
		...
		child = child->NextSib;//child point to Exp
		Operand place = calloc(1, sizeof(Operand_));
		...
		if(strcmp(prePlaceValue, place->u.value)||place->kind != VARIABLE_OP) {
			Operand leftOp = calloc(1, sizeof(Operand_));
			leftOp->kind = VARIABLE_OP;
			strcpy(leftOp->u.value, f->name);
			InterCode assignIR = calloc(1, sizeof(InterCode_));
			assignIR->kind = ASSIGN_IR;
			assignIR->u.unaryOP.left = leftOp;
			assignIR->u.unaryOP.right = place;
			insertCode(assignIR);
		}
...
Type Exp(Node *root, Operand place) {
	...
	if(!strcmp(child->name, "INT")) { //INT
		...
		t->u.basic = INT_TYPE;
		if(place) {
			Operand Int = calloc(1, sizeof(Operand_));
			Int->kind = CONSTANT_OP;
			itoa(child->u.value_i, Int->u.value, 10);
			InterCode intIR = calloc(1, sizeof(InterCode_));
		...
		if(!strcmp(child->name, "ASSIGNOP")) { //Exp ASSIGNOP Exp
			...
			InterCode assignIR1 = calloc(1, sizeof(InterCode_));
			assignIR1->kind = ASSIGN_IR;
			Operand temp1 = calloc(1, sizeof(Operand_));
			temp1->kind = TEMP_VAR_OP;
			leftType = Exp(left, temp1);
			Operand temp2 = calloc(1, sizeof(Operand_));
			if(!strcmp(right->FirstChild->name, "INT")) {
				rightType = Exp(right, NULL);
				temp2->kind = CONSTANT_OP;
				itoa(right->FirstChild->u.value_i, temp2->u.value, 10);
			}
			else {
			...
			if(place) {
				InterCode assignIR2 = calloc(1, sizeof(InterCode_));
			...
			//code0
			if(place) {
				InterCode code0 = calloc(1, sizeof(InterCode_));
				code0->kind = ASSIGN_IR;
				code0->u.binaryOP.left = place;
				Operand zero = cmalloc(1, sizeof(Operand_));
			...
			rightType = Exp(right, NULL);
			Type ret = malloc(sizeof(Type_));
			ret->kind = BASIC;
			ret->u.basic = INT_TYPE;*/
			//code1
			Type code1 = Cond(root, label1, label2);
			//code2
			InterCode temp1 = calloc(1, sizeof(InterCode_));
			temp1->kind = LABEL_IR;
			temp1->u.unaryOP.op = label1;
			insertCode(temp1);
			if(place) {
				InterCode code2 = calloc(1, sizeof(InterCode_));
				code2->kind = ASSIGN_IR;
				code2->u.binaryOP.left = place;
				Operand one = cmalloc(1, sizeof(Operand_));
			...
			//label2
			InterCode temp2 = calloc(1, sizeof(InterCode_));
			temp2->kind = LABEL_IR;
			...
			Node *right = child->NextSib;
			int isConstant1 = 0, isConstant2 = 0;
			Operand temp1 = calloc(1, sizeof(Operand_));
			if(!strcmp(left->FirstChild->name, "INT")) {
				leftType = Exp(left, NULL);
				temp1->kind = CONSTANT_OP;
				itoa(left->FirstChild->u.value_i, temp1->u.value, 10);
				isConstant1 = 1;
			}
			else{
				temp1->kind = TEMP_VAR_OP;
			...
			if(!place) {
				...
				InterCode computeIR = calloc(1, sizeof(InterCode_));
				...
				place->kind = TEMP_VAR_OP;
				place->u.var_no = TempVarCount++;
				if(isConstant1&&isConstant2) { //both constant
					int num1 = left->FirstChild->u.value_i;
					int num2 = right->FirstChild->u.value_i;
					int num;
					switch(computeIR->kind)
					{
						case PLUS_IR:
							num = num1 + num2;
							break;
					...
					Operand temp3 = calloc(1, sizeof(Operand_));
					temp3->kind = CONSTANT_OP;
					sprintf(temp3->u.value, "%d", num);
					computeIR->kind = ASSIGN_IR;
					computeIR->u.binaryOP.left = place;
					computeIR->u.binaryOP.right = temp3;
				}
				else {
			...
			Operand baseOp = calloc(1, sizeof(Operand_));
			...
			Operand indexOp = calloc(1, sizeof(Operand_));
			if(index) {
			...
			Operand offsetOp = calloc(1, sizeof(Operand_));
			offsetOp->kind = TEMP_VAR_OP;
			offsetOp->u.var_no = TempVarCount++;
			if(index) {
				//example a[i][j][k] compute sizeof [j][k]
				Operand numOp = calloc(1, sizeof(Operand_));
				numOp->kind = CONSTANT_OP;
				char *numStr;
				sprintf(numStr, "%d", getSize(leftType->u.array.elem));
				numOp->u.value = numStr;
				
				InterCode offsetIR = calloc(1, sizeof(InterCode_));
				...
				if(leftType->u.array.elem->kind == BASIC) {
					//next is basic
					Operand tempOp = calloc(1, sizeof(Operand_));
		...
		Operand Exp = calloc(1, sizeof(Operand_));
		...
		if(place) {
			...
			zero->kind = CONSTANT_OP;
			strcpy(zero->u.value, "0");
			InterCode minusIR = calloc(1, sizeof(InterCode_));
		...
		if(!strcmp(child->name, "RP")) { //ID LP RP
			...
			if(!strcmp(first->u.value_s, "read")) {
				if(place) {
					InterCode readIR = calloc(1, sizeof(InterCode_));
					readIR->kind = READ_IR;
					place->kind = TEMP_VAR_OP;
					place->u.var_no = TempVarCount++;
				...
				InterCode callIR = calloc(1, sizeof(InterCode_));
				callIR->kind = CALL_IR;
				if(place) {
					callIR->binaryOP.left = place;
				}
				else {
					Operand temp = calloc(1, sizeof(Operand_));
					uselessOp->kind = TEMP_VAR_OP;
					uselessOp->u.var_no = TempVarCount++;
					callIR->binaryOP.left = temp;
				}
				callIR->binaryOP.right = Function;
				insertCode(callIR);
			}
		}
		else { //ID LP Args RP
			...
			if(!isSameFieldList(param, a))
				printf("Error type 9 at Line %d: The function '%s' is not applicable for arguments\n", child->line_no, f->name);*/
			Operand Arglist = calloc(1, sizeof(Operand_));
			Arglist = NULL;
			Args(child, Arglist);
			if(!strcmp(first->u.value_s, "write")) {
				InterCode writeIR = calloc(1, sizeof(InterCode_));
				...
				while(Arglist) {
					InterCode argIR = calloc(1, sizeof(InterCode_));
					argIR->kind = ARG_IR;
...
Type Cond(Node *root, Operand trueLabel, Operand falseLabel) {
	...
	if(!strcmp(child->name, "Exp")) {
		...
		if(!strcmp(mid->name, "RELOP")) { //EXP1 RELOP EXP2
			...
			//label1
			InterCode labelIR = calloc(1, sizeof(InterCode_));
		...
		return Cond(child->NextSib, falseLabelOp, trueLabelOp);
	}
	else { //other cases
		//code1
		...
		if(!strcmp(child->name, "INT")) {
			...
			itoa(child->u.value_i, temp->u.value, 10);
		}
		else {
